# Fries Alert 迁移说明（OpenClaw）

本文档把本次改造整理为“可迁移、低耦合”的交付方案，方便用户在自己的 OpenClaw 项目中落地。

目标：优先使用 `external_skills/Fries_alert` 自包含能力完成部署。

---

## 1. 交付内容（建议直接拷贝）

将以下文件整体拷贝到目标项目的 `external_skills/Fries_alert/`：
...
当前 `FriesAlertSkill` 已具备以下能力：

- 工具名：`send_to_Fries_focus_mode`
- 入参 schema：
  - `mac_address`
  - `sender`
  - `message_summary`
...
- 动态鉴权：
  - 优先 `Fries_ALERT_TOKEN_MAP`
  - 回退 `Fries_ALERT_TOKEN`
- 请求协议：
  - `POST {Fries_BASE_URL}/api/device/{mac}/alert`
...
```env
Fries_BASE_URL=https://your-Fries-host
Fries_ALERT_TOKEN_MAP={"AA:BB:CC:DD:EE:FF":"token_xxx"}
Fries_ALERT_TOKEN=
...
- 仅拷贝 `external_skills/Fries_alert` 目录
...
1. `/api/skills` 能看到 `send_to_Fries_focus_mode`
2. 触发样例日志出现 `FriesAlert start` 与 `sending`
3. 若失败，`FriesAlert done` 能看到具体 `status/detail/error`
4. Fries 后端能收到：
...
- **Q: 工具被调用但没下发？**  
  A: 看 `FriesAlert done` 的 `status/detail/error`，优先排查门控与网络。
