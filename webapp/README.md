...
Fries 的 Next.js 官网与在线刷机前端（App Router）。

- 公网主页: [https://www.www.993636.xyz](https://www.www.993636.xyz)
...
| 变量名 | 默认值 | 用途 |
|--------|--------|------|
| `Fries_BACKEND_API_BASE` | `http://127.0.0.1:8080` | Next.js API Route 的后端代理目标 |
...
当未设置 `NEXT_PUBLIC_FIRMWARE_API_BASE` 时，前端请求当前域名下的 API Route，再由服务端代理到 `Fries_BACKEND_API_BASE`。
...
- `Fries_BACKEND_API_BASE` 指向可访问的 Fries 后端
- 后端已开放 `/api/firmware/*` 相关接口
- 若使用浏览器直连，后端 CORS 已正确配置

## 常见问题

- 刷机页提示 `upstream_unreachable`：检查 `Fries_BACKEND_API_BASE` 是否可达。
